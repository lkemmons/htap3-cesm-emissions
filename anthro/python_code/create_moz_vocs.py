...
This script:
- Reads NMVOC emissions for different sectors
- Applies VOC speciation factors for different regions
- Outputs MOZART species emissions files
"""
...
def create_moz_vocs():
    ...
    factors = np.zeros((nspec, nsectors, nreg))
    
    # Parse the CSV data
    for idx, row in df.iterrows():
        region = str(row[colnames[0]]).replace(' ', '')
        code0 = str(row[colnames[1]]).replace('.', '_')
        code = 'HTAPv3' + code0[4:]
    ...
    # Print factors for verification
    for ireg in range(5):
        print(f"\n{regions[ireg]}")
    ...
    for ispec in range(nspec):
        ...
        if mw <= 0:
            ...
            molwts[ispec] = mw
        print(f"  {species[ispec]}: {molwts[ispec]}")
    ...
    for ispec in range(nspec):
        ...
        for isect in range(nsectors):
            ...
            if sector_name in ds_v.data_vars:
                data_sect = ds_v[sector_name].values
                ...
                print(f"  {sector_name}: {anthro[220, 138, 0]:.6e}")  # Eastern US point
